function XYZ = lph2xyz(p_orbit, sensor_para, line, pixel, h,...
                           ell, CRITERPOS, MAXITER)
...
if size(line) ~= size(pixel)
    error('line and pixel should have same dimensions!');
end
[row, col] = size(line);
equationset = zeros(3, 1, row, col);
solxyz      = zeros(3, row, col);
ApproxCenterOriginal = sensor_para.ApproxCenterOriginal;

VLIGHT = 299792458;             

RSR2x = sensor_para.RSR2x;
tr1 = sensor_para.t_Range1;  
r_time = pix2tr(pixel, tr1, RSR2x);  

prf = sensor_para.line_time_interval;    
ta1 = sensor_para.t_Azi1;  
a_time = line2ta( line,ta1, prf);     
...
%vz=3*Coeff.Z(1).*a_time.^2+2*Coeff.Z(2).*a_time+Coeff.Z(3);
    ...
    X = ones(row, col) * ApproxCenterOriginal.x;
    Y = ones(row, col) * ApproxCenterOriginal.y;
    Z = ones(row, col) * ApproxCenterOriginal.z;
